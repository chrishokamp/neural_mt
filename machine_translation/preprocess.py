#!/usr/bin/env python
# Adapted from:
# https://raw.githubusercontent.com/lisa-groundhog/GroundHog/master/experiments/nmt/preprocess/preprocess.py
...
parser.add_argument("-d", "--dictionary", default='vocab.pkl',
                    help="the name of the pickled binarized text file")
parser.add_argument("-n", "--ngram", type=int, metavar="N", default=None,
                    help="create n-grams")
...
# WORKING: allow creating the vocabulary from another file or set of files
#
...
def open_files(input_files):
    ...
    for i, input_file in enumerate(input_files):
        ...
        if filename.split(os.extsep)[-1] == 'gz':
            input_files[i] = gzip.GzipFile(input_file.name, input_file.mode,
                                          9, input_file)
        elif filename.split(os.extsep)[-1] == 'bz2':
            input_files[i] = bz2.BZ2File(input_file.name, input_file.mode)
        filenames_no_extensions.append(base_filename)
...
def create_dictionary(input_files, filenames_no_extensions):
    # Part I: Counting the words
    counters = []
    sentence_counts = []
    global_counter = Counter()

    # TODO: let user specify a different file for vocab creation (not the file that gets mapped to indices)
    for input_file, base_filename in zip(input_files, filenames_no_extensions):
...
def binarize(input_files, filenames_no_extensions, ngram=None):
    ...
    if ngram is not None:
        ...
        ngrams = numpy.empty((sum(combined_counter.values()) +
                              sum(sentence_counts), ngram),
                             dtype='uint16')
    binarized_corpora = []
    total_ngram_count = 0
    for input_file, base_filename, sentence_count in \
            zip(input_files, filenames_no_extensions, sentence_counts):
        input_filename = os.path.basename(input_file.name)
        ...
        # end for sentence in input_file
...
if __name__ == "__main__":
    ...
    # TODO: this is a mess because functions close over these variables -- make them reusable!
    args = parser.parse_args()
    args_dict = vars(args)
    base_filenames = open_files(args_dict['input'])

    # TODO: WORKING -- allow create dictionary to use different files
    # TODO: to create the source and target vocabs
    # writes the vocabulary files using `safe_pickle`
    combined_counter, sentence_counts, counters, vocab = create_dictionary(args_dict['input'], base_filenames)

    # args.pickle is True by default (store_true)
    if args.ngram or args.pickle:
        binarize(args_dict['input'], base_filenames, ngram=args_dict['ngram'])
